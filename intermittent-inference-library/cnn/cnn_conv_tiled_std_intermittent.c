...
static void _CNN_Intermittent_LayerConv_Tiled_OFM(uint16_t lid, Mat_t* weights, Mat_t* bias, Mat_t* ifm, Mat_t* ofm, ExeParams_t *parE, PreParams_t *parP, uint8_t idxBuf);
static void _CNN_Intermittent_LayerConv_Tiled_IFM(uint16_t lid, Mat_t* weights, Mat_t* bias, Mat_t* ifm, Mat_t* ofm, ExeParams_t *parE, PreParams_t *parP, uint8_t idxBuf);
static void _CNN_Intermittent_LayerConv_Tiled_WEI(uint16_t lid, Mat_t* weights, Mat_t* bias, Mat_t* ifm, Mat_t* ofm, ExeParams_t *parE, PreParams_t *parP, uint8_t idxBuf);
/*******************************************************************************************************************
...
void _CNN_Intermittent_LayerConv_Tiled_OFM(uint16_t lid, Mat_t* weights, Mat_t* bias, Mat_t* ifm, Mat_t* ofm, ExeParams_t *parE, PreParams_t *parP, uint8_t idxBuf){
    ...
    // report progress
    if (row < ofm->h && col < ofm->w && to < ofm->ch && ti < ifm->ch) {
        uint32_t num_tiles_N = ROUND(ifm->ch, parE->Tn),
                 num_tiles_MN = ROUND(ofm->ch, parE->Tm) * num_tiles_N,
                 num_tiles_CMN = ROUND(ofm->w, parE->Tc) * num_tiles_MN;
        uint32_t tile_idx = row / parE->Tr * num_tiles_CMN + col / parE->Tc * num_tiles_MN + to / parE->Tm * num_tiles_N + ti / parE->Tn;
        _DBGUART("P,%d,%d\r\n", lid, tile_idx);
        // _DBGUART("_,%d,%d,%d,%d\r\n", row, col, to, ti);
    }
