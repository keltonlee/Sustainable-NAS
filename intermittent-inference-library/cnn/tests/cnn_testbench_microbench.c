...
void test_microbench_multiple_latency(){
   ...
   for(i=0;i<10;i++){
       ...
       cycleCount += msp_benchmarkStop(TIMER_A1_BASE); _DBGUART("null::,i=%d,cc=%l\r\n", i, (uint32_t)cycleCount); cycleCount = 0;
   }
   _lpm_sleep(5);
   ...
   for(i=0;i<10;i++){
       msp_benchmarkStart(TIMER_A1_BASE, 1);
       result = my_q15mpy(a, b);
       cycleCount += msp_benchmarkStop(TIMER_A1_BASE); _DBGUART("q15mpy::,i=%d,cc=%l\r\n", i, (uint32_t)cycleCount); cycleCount = 0;
   }
   _lpm_sleep(5);
   ...
   for (sz=1; sz <= 128  ; sz+=1){
       ...
       _lpm_sleep(1); // this gives some time for the previous dma/SPI operation to settle down
   }
   _lpm_sleep(2);
   ...
   /****** MathOps - DIV32 ******/
   _DBGUART("-------- MATHOPS_DIV32 \r\n");
   a32=123456;
   b32=1234;
   for(i=0;i<10;i++){
       msp_benchmarkStart(TIMER_A1_BASE, 1);
       result = b32 / a32;
       cycleCount += msp_benchmarkStop(TIMER_A1_BASE); _DBGUART("math_div32::,i=%d,cc=%l\r\n", i, (uint32_t)cycleCount); cycleCount = 0;
   }
   _lpm_sleep(5);
   ...
   /****** MathOps - SHIFT ******/
   _DBGUART("-------- MATHOPS_SHIFT \r\n");
   // force 32-bit shift to match batch norm
   a32 = 123456;
   for(sz=1; sz<=15; sz++){
       msp_benchmarkStart(TIMER_A1_BASE, 1);
       result = a32 << 5;
       cycleCount += msp_benchmarkStop(TIMER_A1_BASE); _DBGUART("math_shift::,sz=%d,cc=%l\r\n", sz, (uint32_t)cycleCount); cycleCount = 0;
   }
   _lpm_sleep(5);
