...
class PlatPerf:
    def __init__(self, NAS_SETTINGS, PLAT_SETTINGS):
        ...
        self.Ecf = 100          # embodied carbon footprint need to fetch from plat_cost_profile
    ...
    def get_inference_times(self, T_cpu, T_mem, T_disk):
        # need to change LT for each component from plat_cost_profile
        LT_cpu = 4000
        LT_mem = 5000
        LT_disk = 6000
        times = min(LT_cpu / T_cpu, LT_mem / T_mem, LT_disk / T_disk)
        return times
    ...
    def get_inference_carbon(self, network, carbon_intensity, fixed_params=None, power_type=None):
        ...
        error = None
        latency = 0
        T_cpu, T_mem, T_disk, energy = 0, 0, 0, 0
        network_exec_design = []        
        ...
        if fixed_params == None:
            ...
            for each_layer in network:             
                ...
                if sols[each_layer['name']]['best_sol'] == None: # cannot find a feasible execution design
                    ...
                    latency = latency + sols[each_layer['name']]['best_sol'][0]['Le2e']
                    T_cpu = T_cpu + sols[each_layer['name']]['best_sol'][0]['T_cpu']
                    T_mem = T_mem + sols[each_layer['name']]['best_sol'][0]['T_mem']
                    T_disk = T_disk + sols[each_layer['name']]['best_sol'][0]['T_disk']
            ...
            for lix, each_layer in enumerate(network): 
                ...
                if cost_stats['Le2e'] != None:
                    latency = latency + cost_stats['Le2e']
                    T_cpu = T_cpu + cost_stats['T_cpu']
                    T_mem = T_mem + cost_stats['T_mem']
                    T_disk = T_disk + cost_stats['T_disk']
        ...
        Ecf_per_inference = self.Ecf / self.get_inference_times(T_cpu, T_mem, T_disk)     # Embodied Carbon Footprint per inference

        carbon = Ocf + Ecf_per_inference
        
        return latency, carbon, network_exec_design, error
    ...
    def get_inference_carbon_verbose(perf_model_intpow, perf_mode_contpow, net_obj, net_config, fixed_params=None):
        try:
            ...
            # -- get perf for CONT pow                 
            # cont pow performance - fixed params : same as intpow        
            e2e_lat_contpow_fp, e2e_carbon_contpow_fp, exec_design_contpow_fp, error_contpow_fp = perf_mode_contpow.get_inference_carbon(net_obj, carbon_intensity = 400, fixed_params=exec_design_intpow)
        ...
        return subnet_latency_info, e2e_carbon_contpow_fp
    ...
    def get_carbon_info(cls, net_obj, net_config, fixed_params=None):
        ...
        subnet_latency_info, CF = cls.get_inference_carbon_verbose(performance_model_intpow, performance_model_contpow, net_obj, net_config, fixed_params=fixed_params)

        return subnet_latency_info, CF
