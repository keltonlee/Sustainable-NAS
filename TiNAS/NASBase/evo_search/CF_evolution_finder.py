...
class EvolutionFinder:
	...
	def __init__(self, global_settings, dataset, supernet, constraint_type, efficiency_constraint, imc_constraint,
	             efficiency_predictor, accuracy_predictor, carbon_predictor, logfname, **kwargs):
		...
		self.carbon_predictor = carbon_predictor
	...
	def check_constraints(self, sample):
		...
		if not self.global_settings.NAS_EVOSEARCH_SETTINGS['EVOSEARCH_BYPASS_EFFICIENCY']:
			...
			cached_carbon = self.evo_memory.query_tbl(sample, EvoMemTypes.CARBON)
			cached_inf_per_lt = self.evo_memory.query_tbl(sample, EvoMemTypes.CARBON)
			# cached_imc = self.evo_memory.query_tbl(sample, EvoMemTypes.CF)
			...
			# in this work just 'CONT' so predict carbon
			if None in [cached_lat, cached_imc, cached_carbon]:	# if any of them are none, then recalculate
				if self.global_settings.PLATFORM_SETTINGS['POW_TYPE'] == 'INT':
					efficiency, imc = self.efficiency_predictor.predict_efficiency_and_imc(sample, self.net_choices, input_ch)
				else:			# 'CONT'
					efficiency, carbon, inf_per_lt, imc = self.carbon_predictor.predict_carbon_efficiency_and_imc(sample, self.net_choices, input_ch)
			else:
				efficiency = cached_lat
				imc = cached_imc    
				carbon = cached_carbon
				inf_per_lt = cached_inf_per_lt
			...
			return sample, efficiency, carbon, inf_per_lt, imc
		else:
			efficiency = carbon = imc = inf_per_lt = -1
			...
			if all_layers_fit_nvm:
				...
				return None, efficiency, carbon, inf_per_lt, imc

	def random_sample(self):
		trials = 0
		while trials < 100:
			sample = self.arch_manager.random_sample()
			sample, efficiency, carbon, inf_per_lt, imc = self.check_constraints(sample)
			if sample:
				return sample, efficiency, carbon, inf_per_lt, imc
			trials += 1
	...
	def mutate_sample(self, sample):
		while True:
			...
			new_sample, efficiency, carbon, inf_per_lt, imc = self.check_constraints(new_sample)
			if new_sample:
				return new_sample, efficiency, carbon, inf_per_lt, imc
	...
	def _mpworker_pop_efficiency(self, worker_id, batched_pop_size):
		...
		for pix in range(batched_pop_size):
			sample, efficiency, carbon, inf_per_lt, imc = self.random_sample()			
			if sample is None:
				break
			child_pool.append(sample); efficiency_pool.append(efficiency); carbon_pool.append(carbon); inf_per_lt_pool.append(inf_per_lt); imc_pool.append(imc)
		...
		result = {
			...
			"inf_per_lt_pool": inf_per_lt_pool,
			"imc_pool": imc_pool,
		}  
		return result
	...
	def _mpworker_pop_mutate(self, worker_id, population, parents_size, batched_mutation_numbers):
		...
		for i in range(batched_mutation_numbers):
			...
			# Mutate
			new_sample, efficiency, carbon, inf_per_lt, imc = self.mutate_sample(par_sample)
			child_pool.append(new_sample); efficiency_pool.append(efficiency); carbon_pool.append(carbon); inf_per_lt_pool.append(inf_per_lt); imc_pool.append(imc)
			#print("_mpworker_pop_mutate::Enter [{}][{}/{}]".format(worker_id, i+1, batched_mutation_numbers))
		result = {
				"child_pool" : child_pool, "efficiency_pool" : efficiency_pool, "carbon_pool" : carbon_pool, "inf_per_lt_pool": inf_per_lt_pool, "imc_pool": imc_pool,
		}  
		return result
	...
	def _mpworker_pop_crossover(self, worker_id, population, parents_size, batched_crossover_numbers):
		...
		for i in range(batched_crossover_numbers):
			...
			# Crossover
			new_sample, efficiency, carbon, inf_per_lt, imc = self.crossover_sample(par_sample1, par_sample2)
			child_pool.append(new_sample); efficiency_pool.append(efficiency); carbon_pool.append(carbon); inf_per_lt_pool.append(inf_per_lt); imc_pool.append(imc)
	...
	def log_progress(self, iteration, population):
		...
		best_acc, _, best_efficiency, best_carbon, best_inf_per_lt, best_imc = best_subnet
		worst_acc, _, worst_efficiency, worst_carbon, worst_inf_per_lt, worst_imc = worst_subnet
		...
		field_names = ["iter", "best_score", "worst_score", "best_acc", "worst_acc", "best_imc", "worst_imc",
				 		"best_efficiency", "worst_efficiency", "best_carbon", "best_inf_per_lt", "best_config", "worst_config", "uniq"]
		field_values = [iteration, best_score, worst_score, best_acc, worst_acc, best_imc, worst_imc,
						best_efficiency, worst_efficiency, best_carbon, best_inf_per_lt, best_info_dict["config"], worst_info_dict["config"], self.calc_unique_subnets(population)]
	...
	# add NSGAII sort function
	def dominates(self, item_a, item_b):
		acc_a, child_a, efficiency_a, cf_a, int_per_lt, imc_a = item_a
		acc_b, child_b, efficiency_b, cf_b, int_per_lt, imc_b = item_b
		# acc_a, cf_a = item_a
		# acc_b, cf_b = item_b
	...
	def crowd_sort(self, population, parents_size):
		crowd_sorted_population = []
		dist = [0] * len(population)

		for obj in [0, 3]:    # we only have two objectives(accuracy and carbon)
	...
	def fast_non_dominated_sort(self, population, parents_size):
		...
		if parents_size > 0:
			...
			sorted_population.extend(self.crowd_sort(last_rank_population, parents_size))
	...
	def get_best_parents(self, parents):
		min_acc = min([parent[0] for parent in parents])
		max_acc = min([parent[0] for parent in parents])
		min_carbon = min([parent[3] for parent in parents])
		max_carbon = min([parent[3] for parent in parents])

		for parent in parents: 
			normalized_acc = self.normalize(parent[0], min_acc, max_acc)
			normalized_carbon = self.normalize(parent[3], min_carbon, max_carbon)
			distance = -normalized_acc**2 + normalized_carbon**2
			parent.append(distance)
		
		best_parent = sorted(parents, key = lambda i: i[-1])
		return best_parent[0][:-1]
	

	# not used in carbon version
	def get_score(self, item):
		acc, child, efficiency, cf, inf_per_lt, imc = item
	...
	def init_population(self, verbose=False):
		...
		population = []  # (validation, sample, latency, carbon, inf_per_lt, imc) tuples
		child_pool = []
		efficiency_pool = []
		carbon_pool = []
		inf_per_lt_pool = []
		...
		if USE_MULTIPROCESSING:	# (### MULTIPROCESSING: CPU)
			...
			for worker_result in all_worker_results:
				...
				carbon_pool.extend(worker_result['carbon_pool'])
				inf_per_lt_pool.extend(worker_result['inf_per_lt_pool'])
			...
			for pix in range(population_size):
				...
				carbon_pool.append(carbon)
				inf_per_lt_pool.append(inf_per_lt)
				imc_pool.append(imc)
		...
		# -- create population
		for pix in range(population_size):
			population.append([accs[pix], child_pool[pix], efficiency_pool[pix], carbon_pool[pix], inf_per_lt_pool[pix], imc_pool[pix]])
  
		
		#self._debug_dump_pop_info(population)
  
		# -- update evo mem
		# need to update carbon in cache?
		for sample, lat, carbon, inf_per_lt, imc, acc in zip(child_pool, efficiency_pool, carbon_pool, inf_per_lt_pool, imc_pool, accs):
			self.evo_memory.update_tbl_multival(sample, 
                                       			[EvoMemTypes.LAT, EvoMemTypes.IMC, EvoMemTypes.ACC, EvoMemTypes.CARBON, EvoMemTypes.INF_PER_LT],
												[lat, imc, acc, carbon, inf_per_lt]
                                          		)
	...
	def run_evolution_search(self, verbose=False):
		...
		best_info = None
		population = []  # (accuracy, sample, latency, carbon, inf_per_lt, imc) tuples
		...
		save_iter_info = []
		save_iter_fname = 'vanila_iter_info.json'
		...
		for iter in tqdm(range(max_time_budget), desc='Searching with %s constraint (%s)' % (self.constraint_type, self.efficiency_constraint)):
			...
			save_parents = [{'acc': parent[0], 'lat': parent[2], 'cf': parent[3], 'inf_per_lt': parent[4]} for parent in parents]
			save_iter_info.append(save_parents)
			# parents = self.fast_non_dominated_sort(population, parents_size)		# NEW NSGAII
  
			best_parent = parents[0]
			# best_parent = self.get_best_parents(parents)
			# print("best parent: ", best_parent)
			# best_acc = best_parent[0]	# accuracy of best parent
			best_score = self.get_score(best_parent)	

			best_acc = max([p[0] for p in parents])
			best_carbon = min([p[3] for p in parents])
			best_inf_per_lt = max([p[4] for p in parents])

			if verbose:
				print('[{}] Iter: {} Best Acc: {} Best Carbon: {} Best inference per lifetime: {}; Best parents: Acc {}, Carbon {}, inference per lifetime {}'.format(datetime.now().strftime("%m/%d/%Y, %H:%M:%S"), 
                                         			iter, best_acc, best_carbon, best_inf_per_lt, best_parent[0], best_parent[3], best_parent[4]))
				

			# update best accuracy in each iteration, if same, then just add the same
			# might need to change since it is multiobjective optimization
			if best_score > best_valids[-1]:
				best_valids.append(best_score)
				best_info = best_parent
			else:
			...
			# -- update evo mem
			for sample, lat, carbon, inf_per_lt, imc in zip(child_pool, efficiency_pool, carbon_pool, inf_per_lt_pool, imc_pool):
				self.evo_memory.update_tbl_multival(sample, 
													[EvoMemTypes.LAT, EvoMemTypes.IMC, EvoMemTypes.CARBON, EvoMemTypes.INF_PER_LT],
													[lat, imc, carbon, inf_per_lt]
													)
			
			
			# ========================= CROSSOVER
			# ---------- crossover a fixed number of random candidates	(### MULTIPROCESSING: CPU)
			# seems like need to subtract len(population)
			...
			for i in range(population_size):
				population.append([accs[i], child_pool[i], efficiency_pool[i], carbon_pool[i], inf_per_lt_pool[i], imc_pool[i]])
		...
		with open(save_iter_fname, "w") as write_file:
			json.dump(save_iter_info, write_file, indent=4)
	...
	def get_metadata_for_best_solution(self, best_info):
		acc, child, efficiency, carbon, inf_per_lt, imc = best_info
		print("best child: ", child)
		subnet_latency_info = self.efficiency_predictor.predict_network_carbon_and_latency_verbose(child, self.net_choices)
		...
		return {
			...
			"accuracy": acc,
			"carbon": carbon,
			"inference_per_lifetime": inf_per_lt,
			"imc": imc,
