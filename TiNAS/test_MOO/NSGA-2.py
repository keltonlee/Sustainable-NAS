...
def dominates(item_a, item_b):
    acc_a, child_a, efficiency_a, cf_a, imc_a = item_a
    acc_b, child_b, efficiency_b, cf_b, imc_b = item_b
    # acc_a, cf_a = item_a
    # acc_b, cf_b = item_b
...
def crowd_sort(population, parents_size):
    ...
    for obj in [0, 3]:    # we only have two objectives(accuracy and carbon) need further change in index 0, 3(acc, cf)
...
# test NSGA-II
population = [(random.randint(70, 100), 0, 0, random.randint(70, 100), 0) for _ in range(1000)]
# print(population)
new_population = fast_non_dominated_sort(population, 50)

x1, _, _, y1, _ = zip(*population)
x2, _, _, y2, _ = zip(*new_population)

plt.scatter(x1, y1, color='blue', label='origin population')
for point in population:
    if point in new_population:
        plt.scatter(point[0], point[3], color='orange')
